...
職缺資料載入完成後，卡片會以 `50ms` 間隔依序淡入並向上移動，讓 Skeleton 與正式內容之間的切換更自然。桌面版卡片在 hover 時會上浮 `3px` 並顯示陰影，手機版則不套用 hover 效果。進場動畫使用 `motion-safe`，使用者偏好減少動態效果時不會播放。

### 職缺詳情

點擊「查看細節」後，`JobDetailDialog` 才依職缺 ID 載入完整內容。彈框包含 Skeleton、錯誤重試、圖片失敗狀態、循環輪播及拖曳操作，也支援背景點擊與 `Escape` 關閉。手機版使用動態視窗高度，避免瀏覽器工具列使彈框超出畫面。手機與桌面版皆加入縮放回彈的進場效果，關閉時縮小並淡出；黑色遮罩則獨立過渡背景色，避免與彈框同時進行透明度合成而產生閃爍。
